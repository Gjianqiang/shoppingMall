...
<script>
  ...
  export default {
    ...
    methods: {
      submit() {
        // 表单校验
        if (this.$refs.brandForm.validate()) {//`validate`方法，进行表单校验。返回boolean值，true代表校验通过
           // 2、定义一个请求参数对象，通过解构表达式来获取brand中的属性
        const {cids ,letter ,...params} = this.brand;
          // 3、数据库中只要保存分类的id即可，因此我们对categories的值进行处理,只保留id，并转为字符串
          console.log('cids', cids.map(c => c.id).join(","))
        params.cids = cids.map(c => c.id).join(",");
        // 4、将字母都处理为大写
